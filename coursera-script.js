const App = {
  ...
  async init() {
    try {
      ...
      this.showToast(
        error.message || "Phiên đăng nhập không hợp lệ. Đang chuyển hướng...",
        "error",
      );
      ...
      if (res.ok) {
        ...
        if (newlyApprovedCourses.length > 0) {
          ...
          newlyApprovedCourses.forEach((courseId) => {
            const c = this.state.courses.find(
              (course) => String(course.id) === String(courseId),
            );
            if (c) {
              this.addNotification(
                "Đơn hàng được duyệt",
                `Lộ trình <span class="text-[#0056D2] dark:text-blue-400 font-bold">${c.title}</span> đã được duyệt. Vào học ngay!`,
                c.id,
              );
            }
          });
  ...
  renderNotifications() {
    ...
    this.state.notifications.forEach((n) => {
      ...
      notificationEl.className = `p-3 border-b border-gray-100 dark:border-slate-700/50 flex items-start gap-3 transition-colors`;
      ...
      if (!n.read) {
        ...
        dot.className = "w-2 h-2 rounded-full bg-blue-500 shrink-0 mt-1.5";
        notificationEl.appendChild(dot);
      }
  ...
  getCourseOrderStatus(courseId) {
    if (
      this.state.user &&
      (this.state.user.role === "admin" || this.state.user.role === "teacher")
    )
      return 3;
    if (Array.isArray(this.state.orders)) {
      const order = this.state.orders.find(
        (o) => String(o.course_name) === String(courseId),
      );
    ...
    staticCategories.forEach((category) => {
      ...
      if (pillsContainer) {
        ...
        if (catValue === "all") isActive = this.state.searchQuery === "";
          ...
          isActive = this.state.searchQuery === catValue.toLowerCase().trim();

        const btnClass = isActive
          ? "bg-[#0056D2] text-white border-[#0056D2] shadow-md shadow-blue-500/20"
          : "bg-white dark:bg-slate-800 text-gray-600 dark:text-gray-300 border-gray-200 dark:border-slate-700 hover:border-gray-300 dark:hover:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-700/50";
      ...
      if (savedState) {
        ...
        if (state.view === "learning" && state.courseId) {
          const course = this.state.courses.find(
            (c) => String(c.id) === String(state.courseId),
          );
          if (course) {
  ...
  renderDashboardCourses() {
    ...
    try {
      let filteredCourses = Array.isArray(this.state.courses)
        ? this.state.courses
        : [];
      ...
      paginatedCourses.forEach((course) => {
        ...
        const completedLessons = courseWeeks.reduce(
          (sum, w) =>
            ...
            (Array.isArray(w.items)
              ? w.items.filter((i) => i.completed).length
              : 0),
          0,
        );
        const progress =
        ...
        if (
          ...
          badgeText.toLowerCase().includes("bán chạy")
        ) {
          badgeClass =
            "bg-gradient-to-r from-red-500 to-orange-500 text-white shadow-md shadow-red-500/30 border border-red-400/30";
          badgeIcon =
            '<i class="fa-solid fa-fire text-yellow-200 mr-1.5 text-xs"></i>';
        } else if (
          badgeText.toLowerCase().includes("mới") ||
          badgeText.toLowerCase().includes("new")
        ) {
          badgeClass =
            "bg-gradient-to-r from-emerald-500 to-green-500 text-white shadow-md shadow-green-500/30 border border-green-400/30";
          badgeIcon =
            '<i class="fa-solid fa-sparkles text-yellow-200 mr-1.5 text-xs"></i>';
        }
  ...
  showLearningView(courseId, forceLock = false, lessonIdToLoad = null) {
    ...
    if (
      String(this.state.currentItemRef.courseId) !== String(courseId) &&
      !lessonIdToLoad
    ) {
    ...
    if (!forceLock)
      sessionStorage.setItem(
        ...
        JSON.stringify({
          view: "learning",
          courseId: courseId,
          lessonId: lessonIdToLoad || this.state.currentItemRef.lessonId,
        }),
      );
    ...
    const unlockedCourses = this.state.courses.filter((c) =>
      this.isCourseUnlocked(c.id),
    );
    ...
    if (this.isCourseUnlocked(course.id) && !forceLock) {
      ...
      if (this.isCourseUnlocked(course.id)) {
        ...
        overlayDesc =
          "Bạn đã đăng ký khóa học này và đang trong quá trình chờ Quản trị viên phê duyệt. Vui lòng quay lại sau.";
      } else {
  ...
  renderCourseNavigation(course, isLocked = false) {
    ...
    courseWeeks.forEach((week) => {
      ...
      weekItems.forEach((item) => {
        ...
        const activeClass =
          !isLocked &&
          String(item.id) === String(this.state.currentItemRef.lessonId)
  ...
  async loadLesson(lessonId) {
    ...
    sessionStorage.setItem(
      ...
      JSON.stringify({
        view: "learning",
        courseId: this.state.currentItemRef.courseId,
        lessonId: lessonId,
      }),
    );
    ...
    if (String(this.state.currentItemRef.lessonId) !== String(lessonId)) return;
  ...
  updateUserInfoDisplay() {
    if (this.state.user) {
      ...
      if (headerAvatar)
        headerAvatar.innerHTML = `<img src="${avatarUrl}" alt="Avatar" class="w-full h-full rounded-full object-cover">`;
      if (dropdownAvatar)
        dropdownAvatar.innerHTML = `<img src="${avatarUrl}" alt="Avatar" class="w-full h-full rounded-full object-cover">`;
  ...
  handleHeaderSearch(e) {
    ...
    this.searchTimeout = setTimeout(() => {
      ...
      if (this.state.searchQuery === val) return;
      ...
      if (this.state.activeView !== "dashboard") {
        ...
        requestAnimationFrame(() => this.renderDashboardCourses());
      }

      if (this.state.activeView === "dashboard" && val.length > 0) {
        this.dom.coursesGrid.scrollIntoView({
          behavior: "smooth",
          block: "start",
        });
      }
  ...
  filterByCategory(category) {
    if (category === "all") {
      ...
      if (this.dom.headerSearchInput)
        this.dom.headerSearchInput.value = category;
    }
    this.state.currentPage = 1;
  ...
  addToCart(courseId) {
    ...
    if (!this.state.cart.includes(courseId)) {
      ...
      toast.className =
        "fixed top-24 left-1/2 transform -translate-x-1/2 bg-green-500 text-white px-4 py-2 rounded-full font-bold shadow-lg z-[9999] animate-fade-in-up text-sm flex items-center gap-2";
  ...
  downloadPdf() {
    this.showToast(
      "Tính năng đang được phát triển. Tài liệu sẽ sớm được cập nhật!",
      "info",
    );
  },
  ...
  async cancelPendingOrder(orderId) {
    showConfirmModal(
      "Bạn có chắc chắn muốn hủy yêu cầu ghi danh này không?",
      async () => {
        try {
  ...
  showToast(message, type = "info") {
    ...
    if (!container) {
      ...
      container.className =
        "fixed top-28 right-5 z-[9999] flex flex-col gap-3 pointer-events-none";
...
async function openAccountModal(targetSubTab = "profile") {
  ...
  const myCourses = App.state.courses.filter((c) => App.isCourseUnlocked(c.id));
  ...
  try {
    ...
    if (res.ok) {
      ...
      localStorage.setItem(
        "coursera_user_session",
        JSON.stringify({ token: data.new_token, user: data.user }),
      );
